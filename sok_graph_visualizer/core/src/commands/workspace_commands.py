...
class SelectWorkspaceCommand(Command):
    ...
    def execute(self) -> Tuple[bool, str]:
        ...
        if not self.workspace_context.select_workspace(self.workspace_id):
...
class DeleteWorkspaceCommand(Command):
    def __init__(self, workspace_context: WorkspaceContext, args: Dict[str, Any]) -> None:
        self.workspace_context = workspace_context
        workspace_id = args.get("workspace_id")
        self.workspace_id = str(workspace_id) if workspace_id is not None else None
    ...
    def execute(self) -> Tuple[bool, str]:
        ...
        if self.workspace_context.workspace_service.get_workspace(self.workspace_id) is None:
            return False, f"Workspace not found: {self.workspace_id}"

        if self.workspace_context.is_last_workspace():
            return False, "There must be at least one workspace"

        try:
            self.workspace_context.workspace_service.remove_workspace(self.workspace_id)
            # If we deleted the active workspace, select another one
...
class SelectVisualizerCommand(Command):
    def __init__(self, workspace_context: WorkspaceContext, plugin_manager: PluginManager, args: Dict[str, Any]) -> None:
        self.workspace_context = workspace_context
        self.plugin_manager = plugin_manager
        self.visualizer_id = args.get("visualizer_id")
        self.visualizer_config = args.get("visualizer_config", {})
    ...
    def execute(self) -> Tuple[bool, str]:
        ...
        if self.workspace_id:
            workspace = self.workspace_context.workspace_service.get_workspace(self.workspace_id)
        else:
            workspace = self.workspace_context.get_active_workspace()
...
class RefreshDataSourceCommand(Command):
    def __init__(self, workspace_context: WorkspaceContext, args: Optional[Dict[str, Any]] = None) -> None:
        self.workspace_context = workspace_context
        self.args = args or {}
